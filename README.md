...
下面我们使用单温任务来描述两个Fourier-DON变体。

### 第一类Fourier-DON：

设 $𝐙∈ℝ^{m\times m}$ 表示域 $\Omega=[0,1]\times[0,1]$ 上离散的电离度函数 $Z$ ， $\xi∈ℝ^p$ 表示输入到主干网络 $T_\theta$ 的离散源函数 $\beta$ 。分支和主干网络的输出为：
...
其中 $c$ 是通道数，在非线性基函数逼近的背景下，每个通道对应一个基函数。分支网络 $B_\theta$ 由多个Fourier层组成，而主干网络 $T_\theta$ 是一个FCN。离散化的目标函数 $𝐄∈ℝ^{m\times m}$ 近似为：
