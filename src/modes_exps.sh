...
# Fourier modes, and random seeds, accepting a device ID as input
# (e.g., ./train_modes.sh device=1)
...
if [ $# -eq 1 ]; then
    ...
    echo "Error: Too many arguments. Expected './train_modes.sh [device=<number>]'"
    exit 1
else
...
for ARCH in "${ARCHS[@]}"; do
    ...
    for MODE in "${MODES[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
            echo "Starting training for task $TASK with arch $ARCH, modes $MODE, seed $SEED on device $DEVICE..."
            ...
            if [ $? -eq 0 ]; then
                echo "Completed training for arch $ARCH, modes $MODE, seed $SEED"
            else
                echo "Error: Training failed for arch $ARCH, modes $MODE, seed $SEED"
            fi
            echo "----------------------------------------"
        done
    done
