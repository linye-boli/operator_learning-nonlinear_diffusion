...
# number of training samples, and random seeds, accepting a device ID as input
# (e.g., ./train_samples.sh device=1)

# Configuration
TASK="heat-2T-zsquares-t1-bmax"
ARCHS=("fdon1" "fdon2")
NUM_TRAINS=(100 200 300 400 500 600)
SEEDS=(0 1 2 3 4)
GRID_SIZE=257
MODES=12
WIDTH=32
...
if [ $# -eq 1 ]; then
    ...
    echo "Error: Too many arguments. Expected './train_samples.sh [device=<number>]'"
    exit 1
else
...
for ARCH in "${ARCHS[@]}"; do
    ...
    for NUM_TRAIN in "${NUM_TRAINS[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
            echo "Starting training for task $TASK with arch $ARCH, num-train $NUM_TRAIN, seed $SEED on device $DEVICE..."
            ...
            if [ $? -eq 0 ]; then
                echo "Completed training for arch $ARCH, num-train $NUM_TRAIN, seed $SEED"
            else
                echo "Error: Training failed for arch $ARCH, num-train $NUM_TRAIN, seed $SEED"
            fi
            echo "----------------------------------------"
        done
    done
